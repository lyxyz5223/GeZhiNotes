import { CustomCanvasProps, EmbeddedCanvasData, LinkBlockInfo } from "@/types/CanvasTypes";
...
const CanvasLinkModule: React.FC<{ props: CustomCanvasProps; extraParams: any }> = ({ props, extraParams }) => {
  // 统一使用 globalData
  const linksInGlobal = props.globalData?.links || [];
  const setLinksInGlobal = props.globalData?.setLinks;
  const color = props.color || '#1976d2';
  const canvases = props.globalData?.canvases || [];

  const [selectedFromCanvasData, setSelectedFromCanvasData] = useState<EmbeddedCanvasData | null>(null);

  // 点击画布处理
  const handleCanvasClick = useCallback((canvas: EmbeddedCanvasData) => {
    if (!selectedFromCanvasData) {
      // 第一次点击，选择源画布
      setSelectedFromCanvasData(canvas);
    } else if (selectedFromCanvasData.id !== canvas.id) {      // 第二次点击，选择目标画布并创建连线
      ...
      const toCanvas = canvas;
        ...
        try {
          [startPoint, endPoint] = calculateCanvasConnectionPoints(fromCanvas, toCanvas);
          // 使用字符串连接方式安全地输出日志
          console.log('计算连线点成功');
        } catch (error) {
